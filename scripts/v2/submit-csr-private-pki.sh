...
#
# The customer CA signs immediately after CSR submission.
...
if [ -z "$ORDER_ID" ] || [ -z "$CSR_FILE" ]; then
    echo "Usage: ORDER_ID=<orderId> CSR_FILE=<path> scripts/v2/submit-csr-private-pki.sh" >&2
    exit 1
fi
...
echo "V2 PUT /api/certinext/v2/private-pki-certificates/$ORDER_ID/csr  csrFile=$CSR_FILE"
curl -s -X PUT "$CERTINEXT_V2_API_URL/api/certinext/v2/private-pki-certificates/$ORDER_ID/csr" \
